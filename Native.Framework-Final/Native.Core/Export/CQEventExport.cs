...
namespace Native.App.Export
{
	...
	{	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("ml.paradis.tool", 1, 9, "酷Q与BDX联动", "0.1.7", 6, "gxh2004", "实现酷Q与BDX服务器端联动,高自定义程度，所有内容、操作、触发器均由config.json定义,注意:必须配合BDX服务端的MCWS插件才能使用", authCode);	
